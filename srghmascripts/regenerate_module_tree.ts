...
const usage = () => {
  console.error(
    [
      "usage:",
      "  bun srghmascripts/regenerate_module_tree.ts <crate> [--depends-on=gen_init::r#gen::Init,gen_std::r#gen::Std]",
      "  bun srghmascripts/regenerate_module_tree.ts gen <crate> [--roots=Init,Lean,Std] [--depends-on=gen_init::r#gen::Init,gen_std::r#gen::Std]",
...
const emitTree = (node: Tree, physicalDir: string, attrBaseDir: string, indent = "") => {
  ...
  for (const [name, child] of children) {
    if (child.children.size === 0) {
      ...
      lines.push(`${indent}#[allow(unsafe_op_in_unsafe_fn)]`);
...
const autogenBanner = "// This file is autogenerated by srghmascripts/regenerate_module_tree.ts. Do not edit manually.";
...
const emitIncludeTree = (sourceRoot: string, roots: string[]) => {
  ...
  const emitDir = async (relDir: string, indent: string): Promise<void> => {
    ...
    const hasDir = relDir && await fs.stat(path.join(sourceRoot, relDir)).then((s) => s.isDirectory()).catch(() => false);
    if (relDir && await fs.stat(indexPath).then((s) => s.isFile()).catch(() => false)) {
      if (!hasDir) {
        const relPath = path.relative(path.join(sourceRoot, relDir), indexPath).replaceAll(path.sep, "/");
        lines.push(`${indent}#[allow(unsafe_op_in_unsafe_fn)]`);
        lines.push(`${indent}#[path = ${JSON.stringify(relPath)}]`);
        lines.push(`${indent}pub mod ${moduleIdent(relDir)};`);
        return;
      }
      const relPath = path.relative(path.join(sourceRoot, relDir), indexPath).replaceAll(path.sep, "/");
      lines.push(`${indent}#[allow(unsafe_op_in_unsafe_fn)]`);
    ...
    for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      if (entry.isDirectory()) {
        ...
        const relPath = path.relative(path.dirname(path.join(sourceRoot, childRel)), path.join(sourceRoot, `${childRel}.rs`)).replaceAll(path.sep, "/");
        lines.push(`${indent}#[allow(unsafe_op_in_unsafe_fn)]`);
        lines.push(`${indent}#[path = ${JSON.stringify(relPath)}]`);
        lines.push(`${indent}pub mod ${childIdent};`);
      }
...
const main = async () => {
  if (targetArg === "gen") {
    ...
    for (const dep of dependsOn) {
      emitter.lines.push(`pub use ${dep};`);
    }
    for (const root of roots) {
      const rootFile = path.join(targetRoot, `${root}.rs`);
      const rootDir = path.join(targetRoot, root);
      const rootHasDir = await fs.stat(rootDir).then((s) => s.isDirectory()).catch(() => false);
      if (await fs.stat(rootFile).then((s) => s.isFile()).catch(() => false) && !rootHasDir) {
        const relPath = path.relative(path.dirname(targetOutput), rootFile).replaceAll(path.sep, "/");
        emitter.lines.push(`#[path = ${JSON.stringify(relPath)}]`);
