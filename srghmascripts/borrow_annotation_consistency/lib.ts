...
/**
 * Strips comments from C/C++ source code while maintaining line counts
 */
export function stripCppComments(content: string): string {
  let result = "";
  let i = 0;
...
export function parseCppFunctions(content: string, filePath: string): CppFn[] {
  const text = stripPreprocessor(stripCppComments(content));
  const out: CppFn[] = [];
