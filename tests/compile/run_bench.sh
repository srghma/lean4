...
if [[ -n $DO_COMPILE ]]; then
  ...
  lean --rust="$1.rs" -Dcompiler.postponeCompile=false "${TEST_LEAN_ARGS[@]}" "$1" || fail "Failed to compile $1 into $1.rs"
  leanc ${LEANC_OPTS-} -O3 -DNDEBUG -o "$1.out" "${TEST_LEANC_ARGS[@]}" "$1.rs" || fail "Failed to compile $1.rs"
