...
@[extern "lean_io_condvar_new"] opaque Condvar.new : BaseIO Condvar
...
structure Channel (α : Type) where
  ...
  partial def forAsync [Inhabited α] (f : α → BaseIO Unit) (ch : Channel α) (prio : Task.Priority := .default) : BaseIO (Task Unit) := do
