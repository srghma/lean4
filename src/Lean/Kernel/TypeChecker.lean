...
public import Lean.Kernel.Declaration
public import Lean.Kernel.Level
public import Lean.Kernel.Quot
public import Lean.Kernel.Inductive.Reduce
public import Lean.Kernel.Instantiate
public import Lean.Kernel.ForEachExprV
public import Lean.Kernel.EquivManager
...
structure TypeChecker.State where
  ...
  whnfCoreCacheRec : ExprMap Expr := {}
  whnfCoreCacheProj : ExprMap Expr := {}
  whnfCoreCacheRecProj : ExprMap Expr := {}
  whnfCache : ExprMap Expr := {}
  eqvManager : EquivManager := {}
  failure : Std.HashSet (Expr × Expr) := {}
  unfold : ExprMap Expr := {}
  recReduce : Std.HashMap Name Nat := {}
  whnfSteps : Nat := 0
...
def reduceRecursor (e : Expr) (cheapRec := false) (cheapProj := false) : RecM (Option Expr) := do
  let env ← getEnv
  if let .const recFn _ := e.getAppFn then
    let n := (← get).recReduce.getD recFn 0 + 1
    modify fun s => { s with recReduce := s.recReduce.insert recFn n }
...
def getWhnfCoreCache (s : State) (cheapRec cheapProj : Bool) : ExprMap Expr :=
  match cheapRec, cheapProj with
  | false, false => s.whnfCoreCache
  | true, false => s.whnfCoreCacheRec
  | false, true => s.whnfCoreCacheProj
  | true, true => s.whnfCoreCacheRecProj

def setWhnfCoreCache (s : State) (cheapRec cheapProj : Bool) (cache : ExprMap Expr) : State :=
  match cheapRec, cheapProj with
  | false, false => { s with whnfCoreCache := cache }
  | true, false => { s with whnfCoreCacheRec := cache }
  | false, true => { s with whnfCoreCacheProj := cache }
  | true, true => { s with whnfCoreCacheRecProj := cache }

def whnfCore' (e : Expr) (cheapRec := false) (cheapProj := false) : RecM Expr := do
  let steps := (← get).whnfSteps + 1
  modify fun s => { s with whnfSteps := steps }
  match e with
  ...
  | .app .. | .letE .. | .proj .. => pure ()
  if let some r := getWhnfCoreCache (← get) cheapRec cheapProj |>.get? e then
    return r
  let rec save r := do
    modify fun s => setWhnfCoreCache s cheapRec cheapProj
      ((getWhnfCoreCache s cheapRec cheapProj).insert e r)
    return r
  match e with
...
def reduceNative (_env : Environment) (e : Expr) : Except Exception (Option Expr) := do
  ...
  if f.isConstOf ``reduceBool then
    throw <| .other s!"lean4lean does not support 'reduceBool {c}' reduction"
  else if f.isConstOf ``reduceNat then
    throw <| .other s!"lean4lean does not support 'reduceNat {c}' reduction"
  return none

def rawNatLitExt? (e : Expr) : Option Nat := if e.isConstOf ``Nat.zero then some 0 else e.rawNatLit?
...
def isDefEqLambda (t s : Expr) (subst : Array Expr := #[]) : RecM Bool :=
  ...
  | .lam _ tDom tBody _, .lam name sDom sBody bi => do
    let sType ← if ptrEqExpr tDom sDom then pure none else
...
def tryUnfoldProjApp (e : Expr) : RecM (Option Expr) := do
  ...
  let e' ← whnfCore e
  return if !ptrEqExpr e' e then e' else none
...
@[inline] def isNatZero (t : Expr) : Bool :=
  t.isConstOf ``Nat.zero || t matches .lit (.natVal 0)
...
def tryStringLitExpansionCore (t s : Expr) : RecM LBool := do
  ...
  unless sf.isConstOf ``String.ofList do return .undef
...
def Methods.withFuel : Nat → Methods
  | 0 =>
    { isDefEqCore := fun _ _ => throw <| .other "deepRecursion: isDefEqCore"
      whnfCore := fun _ _ _ => throw <| .other "deepRecursion: whnfCore"
      whnf := fun _ => throw <| .other "deepRecursion: whnf"
      inferType := fun _ _ => throw <| .other "deepRecursion: inferType" }
  | n + 1 =>
...
def RecM.run (x : RecM α) : M α := x (Methods.withFuel 1000000)
...
def etaExpand (e : Expr) : M Expr :=
  ...
  | it => do
    ...
    | 0, _ => throw <| .other "deepRecursion: etaExpand"
...
@[export lean_kernel_is_def_eq_impl]
def isDefEqImpl (env : Lean.Environment) (lctx : LocalContext) (a b : Expr) :
    Except Lean.Kernel.Exception Bool :=
  M.run env.toKernelEnv (safety := .safe) (lctx := lctx) (lparams := []) do
    isDefEq a b

@[export lean_kernel_whnf_impl]
def whnfImpl (env : Lean.Environment) (lctx : LocalContext) (a : Expr) :
    Except Lean.Kernel.Exception Expr :=
  M.run env.toKernelEnv (safety := .safe) (lctx := lctx) (lparams := []) do
    whnf a

@[export lean_kernel_check_impl]
def checkImpl (env : Lean.Environment) (lctx : LocalContext) (a : Expr) :
    Except Lean.Kernel.Exception Expr :=
  M.run env.toKernelEnv (safety := .safe) (lctx := lctx) (lparams := []) do
    checkType a

def safetyOfBool (unsafeDecl : Bool) : DefinitionSafety :=
  if unsafeDecl then .unsafe else .safe

@[export lean_kernel_env_is_def_eq_impl]
def isDefEqKernelEnvImpl (env : Lean.Kernel.Environment) (lctx : LocalContext)
    (unsafeDecl : Bool) (a b : Expr) : Except Lean.Kernel.Exception Bool :=
  M.run env (safety := safetyOfBool unsafeDecl) (lctx := lctx) (lparams := []) do
    isDefEq a b

@[export lean_kernel_env_whnf_impl]
def whnfKernelEnvImpl (env : Lean.Kernel.Environment) (lctx : LocalContext)
    (unsafeDecl : Bool) (a : Expr) : Except Lean.Kernel.Exception Expr :=
  M.run env (safety := safetyOfBool unsafeDecl) (lctx := lctx) (lparams := []) do
    whnf a

@[export lean_kernel_env_check_impl]
def checkKernelEnvImpl (env : Lean.Kernel.Environment) (lctx : LocalContext)
    (unsafeDecl : Bool) (lparams : List Name) (a : Expr) : Except Lean.Kernel.Exception Expr :=
  M.run env (safety := safetyOfBool unsafeDecl) (lctx := lctx) (lparams := lparams) do
    checkType a
