...
public import Lean.Kernel.PtrEq
...
structure UnionFind where
  parent : Array NodeRef := #[]
  deriving Inhabited

namespace UnionFind

def size (uf : UnionFind) : Nat :=
  uf.parent.size

def push (uf : UnionFind) : UnionFind :=
  { uf with parent := uf.parent.push uf.parent.size }

partial def root (uf : UnionFind) (n : NodeRef) : NodeRef :=
  match uf.parent[n]? with
  | some p => if p == n then n else root uf p
  | none => n

def union (uf : UnionFind) (n₁ n₂ : NodeRef) : UnionFind :=
  let r₁ := uf.root n₁
  let r₂ := uf.root n₂
  if r₁ == r₂ then
    uf
  else if h : r₁ < uf.parent.size then
    { uf with parent := uf.parent.set r₁ r₂ h }
  else
    uf
...
def merge (m : EquivManager) (n1 n2 : NodeRef) : EquivManager :=
  if n1 < m.uf.size && n2 < m.uf.size then
    { m with uf := m.uf.union n1 n2 }
  else
    m
...
partial def isEquiv (e1 e2 : Expr) : StateM EquivManager Bool := do
