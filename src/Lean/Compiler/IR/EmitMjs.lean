...
@[export lean_ir_emit_mjs]
def emitMjs (env : Environment) (modName : Name) : Except String String := do
  dbg_trace env
  match (EmitMjs.main { env := env, modName := modName }).run "" with
