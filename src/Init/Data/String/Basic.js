...
export function lean_string_utf8_byte_size(str) {
...
export function String$decLE(s1, s2) {
  return s1 <= s2;
}
...
export function String$Slice$Pos$nextn(slice, pos, n) {
  const bytes = encoder.encode(slice._1);
  let next = Number(pos);
  let remaining = Number(n);
  while (remaining > 0 && next < bytes.length) {
    const firstByte = bytes[next];
    if (firstByte < 0x80) next += 1;
    else if (firstByte < 0xE0) next += 2;
    else if (firstByte < 0xF0) next += 3;
    else if (firstByte < 0xF8) next += 4;
    else next += 1;
    remaining -= 1;
  }
  return next;
