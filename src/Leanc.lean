...
def main (args : List String) : IO UInt32 := do
  ...
  if hasRs then
    ...
    if let some sourcefile := sourcefile? then
      if args.contains "-shared" then
        IO.eprintln "leanc: shared libraries are not supported in the Rust/Cargo backend"
        return 1
      let crateType :=
        if args.contains "-c" then "rlib"
        else "bin"
      let outfile := match outfile? with
        | some out => out
        | none =>
          let path := System.FilePath.mk sourcefile
          match crateType with
          | "rlib" => "lib" ++ path.fileStem.getD "module" ++ ".rlib"
          | _ => path.withExtension "" |>.toString
      ...
      let rustcArgs := #[s!"--crate-type={crateType}", "--emit=link", "--edition=2021", s!"--crate-name={crateName}", "-C", s!"opt-level={optLevel}", "-C", "debug-assertions=no"] ++ debug ++ #["--extern", s!"lean_runtime={rlib}", "-L", libDir] ++ pkgExterns ++ depsDirArg ++ #["-o", outfile, sourcefile]
