...
mod library_module_impl {
    ...
    // GMP layout: lean_object(8) + __mpz_struct { int(4)+int(4)+mp_limb_t*(8) } = 24 bytes.
    #[repr(C)]
    struct MpzObjectGmp {
        _header: LeanObject,
        _mp_alloc: std::ffi::c_int,
        _mp_size: std::ffi::c_int,
        _mp_d: *mut u64, // mp_limb_t on LP64
    }
    const _: () = assert!(core::mem::size_of::<MpzObjectGmp>() == 24);

    // Non-GMP layout: lean_object(8) + bool(1)+7_pad + size_t(8) + mpn_digit*(8) = 32 bytes.
    #[repr(C)]
    struct MpzObjectNonGmp {
        _header: LeanObject,
        m_sign: bool,  // 1 byte + 7 bytes implicit padding (for size_t alignment)
        m_size: usize, // size_t, always non-negative
        m_digits: *mut u32,
    }
    const _: () = assert!(core::mem::size_of::<MpzObjectNonGmp>() == 32);
    ...
    unsafe fn fix_mpz(o: *mut LeanObject, use_gmp: bool, region: &mut CompactedRegion) {
        if use_gmp {
            ...
            (*mpz)._mp_d = (region.m_begin + (d_rel - region.m_base_addr)) as *mut u64;
            let size = (*mpz)._mp_size.abs() as usize;
            region.m_next = region.m_next.add(align_size(core::mem::size_of::<MpzObjectGmp>() + size * core::mem::size_of::<u64>()));
        } else {
