// Lean compiler output
// Module: Lean.Data.OpenDecl
// Imports: Init.Data.ToString.Name Init.Data.ToString.Extra
use crate::r#gen::Init::Data::List::Basic::l_List_beq___redArg;
use crate::r#gen::Init::Data::ToString::Extra::{
    initialize_Init_Data_ToString_Extra, l_List_toString___redArg,
    runtime_initialize_Init_Data_ToString_Extra,
};
use crate::r#gen::Init::Data::ToString::Name::{
    initialize_Init_Data_ToString_Name, l_Lean_Name_instToString___lam__0,
    l_Lean_Name_toStringWithToken___at___00Lean_Name_toString_spec__0,
    runtime_initialize_Init_Data_ToString_Name,
};
use crate::r#gen::Init::Meta::Defs::l_Lean_Name_replacePrefix;
use crate::r#gen::Init::Prelude::{l_Lean_Name_beq___boxed, l_Lean_Name_mkStr1};
use crate::lean_imports_rs::Init::Data::String::Defs::lean_string_append;
use crate::lean_imports_rs::Init::Prelude::lean_name_eq;
use crate::leanh::{
    LeanArrayObject, LeanClosureObject, LeanCtorObject, LeanExternalClass, LeanExternalObject,
    LeanObject, LeanOnceCell, LeanPromiseObject, LeanRefObject, LeanScalarArray, LeanStringObject,
    LeanTaskObject, LeanThunkObject, lean_apply_2, lean_box, lean_ctor_get, lean_dec, lean_dec_ref,
    lean_dec_ref_known, lean_inc, lean_inc_n, lean_io_result_is_error, lean_io_result_mk_ok,
    lean_obj_tag, lean_unsigned_to_nat,
};
pub static l_Lean_instBEqOpenDecl___closed__0_value: LeanClosureObject<0> = LeanClosureObject {
    m_header: LeanObject {
        rc: 0,
        cs_size: (core::mem::size_of::<LeanObject>()
            + core::mem::size_of::<*const core::ffi::c_void>()
            + 4
            + core::mem::size_of::<*mut LeanObject>() * 0) as u16,
        other: 0,
        tag: 245,
    },
    m_fun: l_Lean_instBEqOpenDecl_beq___boxed as *const core::ffi::c_void,
    m_arity: 2,
    m_num_fixed: 0,
    m_objs: [],
};
static mut l_Lean_instBEqOpenDecl___closed__0: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_instBEqOpenDecl___closed__0_value) as *mut LeanObject;
pub static mut l_Lean_instBEqOpenDecl: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_instBEqOpenDecl___closed__0_value) as *mut LeanObject;
pub static l_Lean_OpenDecl_instInhabited___closed__0_value: LeanCtorObject<2> = LeanCtorObject {
    m_header: LeanObject {
        rc: 0,
        cs_size: (core::mem::size_of::<LeanObject>()
            + core::mem::size_of::<*mut LeanObject>() * 2
            + 0) as u16,
        other: 2,
        tag: 0,
    },
    m_objs: [
        (((0 as usize) << 1) | 1) as *mut LeanObject,
        (((0 as usize) << 1) | 1) as *mut LeanObject,
    ],
};
static mut l_Lean_OpenDecl_instInhabited___closed__0: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_OpenDecl_instInhabited___closed__0_value) as *mut LeanObject;
pub static mut l_Lean_OpenDecl_instInhabited: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_OpenDecl_instInhabited___closed__0_value) as *mut LeanObject;
pub static l_Lean_OpenDecl_instToString___lam__0___closed__0_value: LeanStringObject<9> =
    LeanStringObject {
        m_header: LeanObject {
            rc: 0,
            cs_size: (0) as u16,
            other: 0,
            tag: 249,
        },
        m_size: 9,
        m_capacity: 9,
        m_length: 8,
        m_data: [32, 104, 105, 100, 105, 110, 103, 32, 0],
    };
static mut l_Lean_OpenDecl_instToString___lam__0___closed__0: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_OpenDecl_instToString___lam__0___closed__0_value) as *mut LeanObject;
pub static l_Lean_OpenDecl_instToString___lam__0___closed__1_value: LeanStringObject<6> =
    LeanStringObject {
        m_header: LeanObject {
            rc: 0,
            cs_size: (0) as u16,
            other: 0,
            tag: 249,
        },
        m_size: 6,
        m_capacity: 6,
        m_length: 3,
        m_data: [32, 226, 134, 146, 32, 0],
    };
static mut l_Lean_OpenDecl_instToString___lam__0___closed__1: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_OpenDecl_instToString___lam__0___closed__1_value) as *mut LeanObject;
pub static l_Lean_OpenDecl_instToString___closed__0_value: LeanClosureObject<0> =
    LeanClosureObject {
        m_header: LeanObject {
            rc: 0,
            cs_size: (core::mem::size_of::<LeanObject>()
                + core::mem::size_of::<*const core::ffi::c_void>()
                + 4
                + core::mem::size_of::<*mut LeanObject>() * 0) as u16,
            other: 0,
            tag: 245,
        },
        m_fun: l_Lean_Name_instToString___lam__0 as *const core::ffi::c_void,
        m_arity: 1,
        m_num_fixed: 0,
        m_objs: [],
    };
static mut l_Lean_OpenDecl_instToString___closed__0: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_OpenDecl_instToString___closed__0_value) as *mut LeanObject;
pub static l_Lean_OpenDecl_instToString___closed__1_value: LeanClosureObject<0> =
    LeanClosureObject {
        m_header: LeanObject {
            rc: 0,
            cs_size: (core::mem::size_of::<LeanObject>()
                + core::mem::size_of::<*const core::ffi::c_void>()
                + 4
                + core::mem::size_of::<*mut LeanObject>() * 0) as u16,
            other: 0,
            tag: 245,
        },
        m_fun: l_Lean_Name_beq___boxed as *const core::ffi::c_void,
        m_arity: 2,
        m_num_fixed: 0,
        m_objs: [],
    };
static mut l_Lean_OpenDecl_instToString___closed__1: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_OpenDecl_instToString___closed__1_value) as *mut LeanObject;
pub static l_Lean_OpenDecl_instToString___closed__2_value: LeanClosureObject<2> =
    LeanClosureObject {
        m_header: LeanObject {
            rc: 0,
            cs_size: (core::mem::size_of::<LeanObject>()
                + core::mem::size_of::<*const core::ffi::c_void>()
                + 4
                + core::mem::size_of::<*mut LeanObject>() * 2) as u16,
            other: 0,
            tag: 245,
        },
        m_fun: l_Lean_OpenDecl_instToString___lam__0 as *const core::ffi::c_void,
        m_arity: 3,
        m_num_fixed: 2,
        m_objs: [
            core::ptr::addr_of!(l_Lean_OpenDecl_instToString___closed__1_value) as *mut LeanObject,
            core::ptr::addr_of!(l_Lean_OpenDecl_instToString___closed__0_value) as *mut LeanObject,
        ],
    };
static mut l_Lean_OpenDecl_instToString___closed__2: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_OpenDecl_instToString___closed__2_value) as *mut LeanObject;
pub static mut l_Lean_OpenDecl_instToString: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_OpenDecl_instToString___closed__2_value) as *mut LeanObject;
pub static l_Lean_rootNamespace___closed__0_value: LeanStringObject<7> = LeanStringObject {
    m_header: LeanObject {
        rc: 0,
        cs_size: (0) as u16,
        other: 0,
        tag: 249,
    },
    m_size: 7,
    m_capacity: 7,
    m_length: 6,
    m_data: [95, 114, 111, 111, 116, 95, 0],
};
static mut l_Lean_rootNamespace___closed__0: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_rootNamespace___closed__0_value) as *mut LeanObject;
pub static l_Lean_rootNamespace___closed__1_value: LeanCtorObject<3> = LeanCtorObject {
    m_header: LeanObject {
        rc: 0,
        cs_size: (core::mem::size_of::<LeanObject>()
            + core::mem::size_of::<*mut LeanObject>() * 2
            + 8) as u16,
        other: 2,
        tag: 1,
    },
    m_objs: [
        (((0 as usize) << 1) | 1) as *mut LeanObject,
        core::ptr::addr_of!(l_Lean_rootNamespace___closed__0_value) as *mut LeanObject,
        626731335300788152 as *mut LeanObject,
    ],
};
static mut l_Lean_rootNamespace___closed__1: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_rootNamespace___closed__1_value) as *mut LeanObject;
pub static mut l_Lean_rootNamespace: *mut LeanObject =
    core::ptr::addr_of!(l_Lean_rootNamespace___closed__1_value) as *mut LeanObject;
pub unsafe fn l_Lean_OpenDecl_ctorIdx(mut v_x_117_: *mut LeanObject) -> *mut LeanObject {
    if lean_obj_tag(v_x_117_) == 0 {
        let mut v___x_118_: *mut LeanObject = core::ptr::null_mut();
        v___x_118_ = lean_unsigned_to_nat(0);
        return v___x_118_;
    } else {
        let mut v___x_119_: *mut LeanObject = core::ptr::null_mut();
        v___x_119_ = lean_unsigned_to_nat(1);
        return v___x_119_;
    }
}
pub unsafe fn l_Lean_OpenDecl_ctorIdx___boxed(mut v_x_120_: *mut LeanObject) -> *mut LeanObject {
    let mut v_res_121_: *mut LeanObject = core::ptr::null_mut();
    v_res_121_ = l_Lean_OpenDecl_ctorIdx(v_x_120_);
    lean_dec_ref(v_x_120_);
    return v_res_121_;
}
pub unsafe fn l_Lean_OpenDecl_ctorElim___redArg(
    mut v_t_122_: *mut LeanObject,
    mut v_k_123_: *mut LeanObject,
) -> *mut LeanObject {
    let mut v_ns_124_: *mut LeanObject = core::ptr::null_mut();
    let mut v_except_125_: *mut LeanObject = core::ptr::null_mut();
    let mut v___x_126_: *mut LeanObject = core::ptr::null_mut();
    v_ns_124_ = lean_ctor_get(v_t_122_, 0);
    lean_inc(v_ns_124_);
    v_except_125_ = lean_ctor_get(v_t_122_, 1);
    lean_inc(v_except_125_);
    lean_dec_ref(v_t_122_);
    v___x_126_ = lean_apply_2(v_k_123_, v_ns_124_, v_except_125_);
    return v___x_126_;
}
pub unsafe fn l_Lean_OpenDecl_ctorElim(
    mut v_motive_127_: *mut LeanObject,
    mut v_ctorIdx_128_: *mut LeanObject,
    mut v_t_129_: *mut LeanObject,
    mut v_h_130_: *mut LeanObject,
    mut v_k_131_: *mut LeanObject,
) -> *mut LeanObject {
    let mut v___x_132_: *mut LeanObject = core::ptr::null_mut();
    v___x_132_ = l_Lean_OpenDecl_ctorElim___redArg(v_t_129_, v_k_131_);
    return v___x_132_;
}
pub unsafe fn l_Lean_OpenDecl_ctorElim___boxed(
    mut v_motive_133_: *mut LeanObject,
    mut v_ctorIdx_134_: *mut LeanObject,
    mut v_t_135_: *mut LeanObject,
    mut v_h_136_: *mut LeanObject,
    mut v_k_137_: *mut LeanObject,
) -> *mut LeanObject {
    let mut v_res_138_: *mut LeanObject = core::ptr::null_mut();
    v_res_138_ =
        l_Lean_OpenDecl_ctorElim(v_motive_133_, v_ctorIdx_134_, v_t_135_, v_h_136_, v_k_137_);
    lean_dec(v_ctorIdx_134_);
    return v_res_138_;
}
pub unsafe fn l_Lean_OpenDecl_simple_elim___redArg(
    mut v_t_139_: *mut LeanObject,
    mut v_simple_140_: *mut LeanObject,
) -> *mut LeanObject {
    let mut v___x_141_: *mut LeanObject = core::ptr::null_mut();
    v___x_141_ = l_Lean_OpenDecl_ctorElim___redArg(v_t_139_, v_simple_140_);
    return v___x_141_;
}
pub unsafe fn l_Lean_OpenDecl_simple_elim(
    mut v_motive_142_: *mut LeanObject,
    mut v_t_143_: *mut LeanObject,
    mut v_h_144_: *mut LeanObject,
    mut v_simple_145_: *mut LeanObject,
) -> *mut LeanObject {
    let mut v___x_146_: *mut LeanObject = core::ptr::null_mut();
    v___x_146_ = l_Lean_OpenDecl_ctorElim___redArg(v_t_143_, v_simple_145_);
    return v___x_146_;
}
pub unsafe fn l_Lean_OpenDecl_explicit_elim___redArg(
    mut v_t_147_: *mut LeanObject,
    mut v_explicit_148_: *mut LeanObject,
) -> *mut LeanObject {
    let mut v___x_149_: *mut LeanObject = core::ptr::null_mut();
    v___x_149_ = l_Lean_OpenDecl_ctorElim___redArg(v_t_147_, v_explicit_148_);
    return v___x_149_;
}
pub unsafe fn l_Lean_OpenDecl_explicit_elim(
    mut v_motive_150_: *mut LeanObject,
    mut v_t_151_: *mut LeanObject,
    mut v_h_152_: *mut LeanObject,
    mut v_explicit_153_: *mut LeanObject,
) -> *mut LeanObject {
    let mut v___x_154_: *mut LeanObject = core::ptr::null_mut();
    v___x_154_ = l_Lean_OpenDecl_ctorElim___redArg(v_t_151_, v_explicit_153_);
    return v___x_154_;
}
pub unsafe fn l_List_beq___at___00Lean_instBEqOpenDecl_beq_spec__0(
    mut v_x_155_: *mut LeanObject,
    mut v_x_156_: *mut LeanObject,
) -> u8 {
    let mut v___x_157_: u8 = 0;
    let mut v___x_158_: u8 = 0;
    let mut v___x_159_: u8 = 0;
    let mut v_head_160_: *mut LeanObject = core::ptr::null_mut();
    let mut v_tail_161_: *mut LeanObject = core::ptr::null_mut();
    let mut v_head_162_: *mut LeanObject = core::ptr::null_mut();
    let mut v_tail_163_: *mut LeanObject = core::ptr::null_mut();
    let mut v___x_164_: u8 = 0;
    let mut state = 0;
    loop {
        match state {
            0 => {
                if lean_obj_tag(v_x_155_) == 0 {
                    if lean_obj_tag(v_x_156_) == 0 {
                        v___x_157_ = 1;
                        return v___x_157_;
                    } else {
                        v___x_158_ = 0;
                        return v___x_158_;
                    }
                } else {
                    if lean_obj_tag(v_x_156_) == 0 {
                        v___x_159_ = 0;
                        return v___x_159_;
                    } else {
                        v_head_160_ = lean_ctor_get(v_x_155_, 0);
                        v_tail_161_ = lean_ctor_get(v_x_155_, 1);
                        v_head_162_ = lean_ctor_get(v_x_156_, 0);
                        v_tail_163_ = lean_ctor_get(v_x_156_, 1);
                        v___x_164_ = lean_name_eq(v_head_160_, v_head_162_);
                        if v___x_164_ == 0 {
                            return v___x_164_;
                        } else {
                            v_x_155_ = v_tail_161_;
                            v_x_156_ = v_tail_163_;
                            state = 0;
                            continue;
                        }
                    }
                }
            }
            _ => {}
        }
    }
}
pub unsafe fn l_List_beq___at___00Lean_instBEqOpenDecl_beq_spec__0___boxed(
    mut v_x_166_: *mut LeanObject,
    mut v_x_167_: *mut LeanObject,
) -> *mut LeanObject {
    let mut v_res_168_: u8 = 0;
    let mut v_r_169_: *mut LeanObject = core::ptr::null_mut();
    v_res_168_ = l_List_beq___at___00Lean_instBEqOpenDecl_beq_spec__0(v_x_166_, v_x_167_);
    lean_dec(v_x_167_);
    lean_dec(v_x_166_);
    v_r_169_ = lean_box((v_res_168_) as usize);
    return v_r_169_;
}
pub unsafe fn l_Lean_instBEqOpenDecl_beq(
    mut v_x_170_: *mut LeanObject,
    mut v_x_171_: *mut LeanObject,
) -> u8 {
    if lean_obj_tag(v_x_170_) == 0 {
        if lean_obj_tag(v_x_171_) == 0 {
            let mut v_ns_172_: *mut LeanObject = core::ptr::null_mut();
            let mut v_except_173_: *mut LeanObject = core::ptr::null_mut();
            let mut v_ns_174_: *mut LeanObject = core::ptr::null_mut();
            let mut v_except_175_: *mut LeanObject = core::ptr::null_mut();
            let mut v___x_176_: u8 = 0;
            v_ns_172_ = lean_ctor_get(v_x_170_, 0);
            v_except_173_ = lean_ctor_get(v_x_170_, 1);
            v_ns_174_ = lean_ctor_get(v_x_171_, 0);
            v_except_175_ = lean_ctor_get(v_x_171_, 1);
            v___x_176_ = lean_name_eq(v_ns_172_, v_ns_174_);
            if v___x_176_ == 0 {
                return v___x_176_;
            } else {
                let mut v___x_177_: u8 = 0;
                v___x_177_ = l_List_beq___at___00Lean_instBEqOpenDecl_beq_spec__0(
                    v_except_173_,
                    v_except_175_,
                );
                return v___x_177_;
            }
        } else {
            let mut v___x_178_: u8 = 0;
            v___x_178_ = 0;
            return v___x_178_;
        }
    } else {
        if lean_obj_tag(v_x_171_) == 1 {
            let mut v_id_179_: *mut LeanObject = core::ptr::null_mut();
            let mut v_declName_180_: *mut LeanObject = core::ptr::null_mut();
            let mut v_id_181_: *mut LeanObject = core::ptr::null_mut();
            let mut v_declName_182_: *mut LeanObject = core::ptr::null_mut();
            let mut v___x_183_: u8 = 0;
            v_id_179_ = lean_ctor_get(v_x_170_, 0);
            v_declName_180_ = lean_ctor_get(v_x_170_, 1);
            v_id_181_ = lean_ctor_get(v_x_171_, 0);
            v_declName_182_ = lean_ctor_get(v_x_171_, 1);
            v___x_183_ = lean_name_eq(v_id_179_, v_id_181_);
            if v___x_183_ == 0 {
                return v___x_183_;
            } else {
                let mut v___x_184_: u8 = 0;
                v___x_184_ = lean_name_eq(v_declName_180_, v_declName_182_);
                return v___x_184_;
            }
        } else {
            let mut v___x_185_: u8 = 0;
            v___x_185_ = 0;
            return v___x_185_;
        }
    }
}
pub unsafe fn l_Lean_instBEqOpenDecl_beq___boxed(
    mut v_x_186_: *mut LeanObject,
    mut v_x_187_: *mut LeanObject,
) -> *mut LeanObject {
    let mut v_res_188_: u8 = 0;
    let mut v_r_189_: *mut LeanObject = core::ptr::null_mut();
    v_res_188_ = l_Lean_instBEqOpenDecl_beq(v_x_186_, v_x_187_);
    lean_dec_ref(v_x_187_);
    lean_dec_ref(v_x_186_);
    v_r_189_ = lean_box((v_res_188_) as usize);
    return v_r_189_;
}
pub unsafe fn l_Lean_OpenDecl_instToString___lam__0(
    mut v___x_198_: *mut LeanObject,
    mut v___f_199_: *mut LeanObject,
    mut v_decl_200_: *mut LeanObject,
) -> *mut LeanObject {
    if lean_obj_tag(v_decl_200_) == 0 {
        let mut v_ns_201_: *mut LeanObject = core::ptr::null_mut();
        let mut v_except_202_: *mut LeanObject = core::ptr::null_mut();
        let mut v___x_203_: u8 = 0;
        let mut v___x_204_: *mut LeanObject = core::ptr::null_mut();
        let mut v___x_205_: *mut LeanObject = core::ptr::null_mut();
        let mut v___x_206_: u8 = 0;
        v_ns_201_ = lean_ctor_get(v_decl_200_, 0);
        lean_inc(v_ns_201_);
        v_except_202_ = lean_ctor_get(v_decl_200_, 1);
        lean_inc_n(v_except_202_, 2);
        lean_dec_ref_known(v_decl_200_, 2);
        v___x_203_ = 1;
        v___x_204_ = l_Lean_Name_toStringWithToken___at___00Lean_Name_toString_spec__0(
            v_ns_201_, v___x_203_,
        );
        v___x_205_ = lean_box(0);
        v___x_206_ = l_List_beq___redArg(v___x_198_, v_except_202_, v___x_205_);
        if v___x_206_ == 0 {
            let mut v___x_207_: *mut LeanObject = core::ptr::null_mut();
            let mut v___x_208_: *mut LeanObject = core::ptr::null_mut();
            let mut v___x_209_: *mut LeanObject = core::ptr::null_mut();
            let mut v___x_210_: *mut LeanObject = core::ptr::null_mut();
            v___x_207_ = l_Lean_OpenDecl_instToString___lam__0___closed__0;
            v___x_208_ = l_List_toString___redArg(v___f_199_, v_except_202_);
            v___x_209_ = lean_string_append(v___x_207_, v___x_208_);
            lean_dec_ref(v___x_208_);
            v___x_210_ = lean_string_append(v___x_204_, v___x_209_);
            lean_dec_ref(v___x_209_);
            return v___x_210_;
        } else {
            lean_dec(v_except_202_);
            lean_dec_ref(v___f_199_);
            return v___x_204_;
        }
    } else {
        let mut v_id_211_: *mut LeanObject = core::ptr::null_mut();
        let mut v_declName_212_: *mut LeanObject = core::ptr::null_mut();
        let mut v___x_213_: u8 = 0;
        let mut v___x_214_: *mut LeanObject = core::ptr::null_mut();
        let mut v___x_215_: *mut LeanObject = core::ptr::null_mut();
        let mut v___x_216_: *mut LeanObject = core::ptr::null_mut();
        let mut v___x_217_: *mut LeanObject = core::ptr::null_mut();
        let mut v___x_218_: *mut LeanObject = core::ptr::null_mut();
        lean_dec_ref(v___f_199_);
        lean_dec_ref(v___x_198_);
        v_id_211_ = lean_ctor_get(v_decl_200_, 0);
        lean_inc(v_id_211_);
        v_declName_212_ = lean_ctor_get(v_decl_200_, 1);
        lean_inc(v_declName_212_);
        lean_dec_ref_known(v_decl_200_, 2);
        v___x_213_ = 1;
        v___x_214_ = l_Lean_Name_toStringWithToken___at___00Lean_Name_toString_spec__0(
            v_id_211_, v___x_213_,
        );
        v___x_215_ = l_Lean_OpenDecl_instToString___lam__0___closed__1;
        v___x_216_ = lean_string_append(v___x_214_, v___x_215_);
        v___x_217_ = l_Lean_Name_toStringWithToken___at___00Lean_Name_toString_spec__0(
            v_declName_212_,
            v___x_213_,
        );
        v___x_218_ = lean_string_append(v___x_216_, v___x_217_);
        lean_dec_ref(v___x_217_);
        return v___x_218_;
    }
}
pub unsafe fn l_Lean_removeRoot(mut v_n_229_: *mut LeanObject) -> *mut LeanObject {
    let mut v___x_230_: *mut LeanObject = core::ptr::null_mut();
    let mut v___x_231_: *mut LeanObject = core::ptr::null_mut();
    let mut v___x_232_: *mut LeanObject = core::ptr::null_mut();
    v___x_230_ = l_Lean_rootNamespace;
    v___x_231_ = lean_box(0);
    v___x_232_ = l_Lean_Name_replacePrefix(v_n_229_, v___x_230_, v___x_231_);
    return v___x_232_;
}
static mut _G_runtime_initialized: bool = false;
pub unsafe fn runtime_initialize_Lean_Data_OpenDecl(builtin: u8) -> *mut LeanObject {
    let mut res: *mut LeanObject = core::ptr::null_mut();
    if _G_runtime_initialized {
        return lean_io_result_mk_ok(lean_box(0));
    }
    _G_runtime_initialized = true;
    res = runtime_initialize_Init_Data_ToString_Name(builtin);
    if lean_io_result_is_error(res) {
        return res;
    }
    lean_dec_ref(res);
    res = runtime_initialize_Init_Data_ToString_Extra(builtin);
    if lean_io_result_is_error(res) {
        return res;
    }
    lean_dec_ref(res);
    return lean_io_result_mk_ok(lean_box(0));
}
static mut _G_meta_initialized: bool = false;
pub unsafe fn meta_initialize_Lean_Data_OpenDecl(builtin: u8) -> *mut LeanObject {
    let mut res: *mut LeanObject = core::ptr::null_mut();
    if _G_meta_initialized {
        return lean_io_result_mk_ok(lean_box(0));
    }
    _G_meta_initialized = true;
    return lean_io_result_mk_ok(lean_box(0));
}
static mut _G_initialized: bool = false;
pub unsafe fn initialize_Lean_Data_OpenDecl(builtin: u8) -> *mut LeanObject {
    let mut res: *mut LeanObject = core::ptr::null_mut();
    if _G_initialized {
        return lean_io_result_mk_ok(lean_box(0));
    }
    _G_initialized = true;
    res = initialize_Init_Data_ToString_Name(builtin);
    if lean_io_result_is_error(res) {
        return res;
    }
    lean_dec_ref(res);
    res = initialize_Init_Data_ToString_Extra(builtin);
    if lean_io_result_is_error(res) {
        return res;
    }
    lean_dec_ref(res);
    res = runtime_initialize_Lean_Data_OpenDecl(builtin);
    if lean_io_result_is_error(res) {
        return res;
    }
    lean_dec_ref(res);
    res = meta_initialize_Lean_Data_OpenDecl(builtin);
    if lean_io_result_is_error(res) {
        return res;
    }
    lean_dec_ref(res);
    return initialize_Lean_Data_OpenDecl(builtin);
}
