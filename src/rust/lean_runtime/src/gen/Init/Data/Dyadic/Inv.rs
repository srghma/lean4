...
pub unsafe fn l_Dyadic_divAtPrec(
    ...
    mut v_prec_30_: *mut LeanObject,
) -> *mut LeanObject {
    if lean_obj_tag(v_b_29_) == 0 {
        lean_dec(v_a_28_);
        return v_b_29_;
    } else {
...
pub unsafe fn runtime_initialize_Init_Data_Dyadic_Inv(builtin: u8) -> *mut LeanObject {
...
pub unsafe fn meta_initialize_Init_Data_Dyadic_Inv(builtin: u8) -> *mut LeanObject {
    let mut res: *mut LeanObject = core::ptr::null_mut();
...
pub unsafe fn initialize_Init_Data_Dyadic_Inv(builtin: u8) -> *mut LeanObject {
    let mut res: *mut LeanObject = core::ptr::null_mut();
    if _G_initialized {
        return lean_io_result_mk_ok(lean_box(0));
    }
