// Lean compiler output
// Module: Init.Data.Vector.Zip
// Imports: Init.Data.Array.Basic Init.Data.Vector.Basic Init.Data.Function Init.Data.Vector.Basic Init.Data.Array.Zip Init.Data.Vector.Lemmas
use crate::r#gen::Init::Data::Array::Basic::{
    initialize_Init_Data_Array_Basic, runtime_initialize_Init_Data_Array_Basic,
};
use crate::r#gen::Init::Data::Array::Zip::{
    initialize_Init_Data_Array_Zip, runtime_initialize_Init_Data_Array_Zip,
};
use crate::r#gen::Init::Data::Function::{
    initialize_Init_Data_Function, runtime_initialize_Init_Data_Function,
};
use crate::r#gen::Init::Data::Vector::Basic::{
    initialize_Init_Data_Vector_Basic, runtime_initialize_Init_Data_Vector_Basic,
};
use crate::r#gen::Init::Data::Vector::Lemmas::{
    initialize_Init_Data_Vector_Lemmas, runtime_initialize_Init_Data_Vector_Lemmas,
};
pub unsafe fn l___private_Init_Data_Vector_Zip_0__Array_getElem_x3f__zipWith_match__1_splitter___redArg(
    mut v_x_43_: *mut crate::leanh::LeanObject,
    mut v_x_44_: *mut crate::leanh::LeanObject,
    mut v_h__1_45_: *mut crate::leanh::LeanObject,
    mut v_h__2_46_: *mut crate::leanh::LeanObject,
) -> *mut crate::leanh::LeanObject {
    if crate::leanh::lean_obj_tag(v_x_43_) == 1 {
        if crate::leanh::lean_obj_tag(v_x_44_) == 1 {
            let mut v_val_47_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            let mut v_val_48_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            let mut v___x_49_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            crate::leanh::lean_dec(v_h__2_46_);
            v_val_47_ = crate::leanh::lean_ctor_get(v_x_43_, 0);
            crate::leanh::lean_inc(v_val_47_);
            crate::leanh::lean_dec_ref_known(v_x_43_, 1);
            v_val_48_ = crate::leanh::lean_ctor_get(v_x_44_, 0);
            crate::leanh::lean_inc(v_val_48_);
            crate::leanh::lean_dec_ref_known(v_x_44_, 1);
            v___x_49_ = crate::leanh::lean_apply_2(v_h__1_45_, v_val_47_, v_val_48_);
            return v___x_49_;
        } else {
            let mut v___x_50_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            crate::leanh::lean_dec(v_h__1_45_);
            v___x_50_ =
                crate::leanh::lean_apply_3(v_h__2_46_, v_x_43_, v_x_44_, crate::leanh::lean_box(0));
            return v___x_50_;
        }
    } else {
        let mut v___x_51_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
        crate::leanh::lean_dec(v_h__1_45_);
        v___x_51_ =
            crate::leanh::lean_apply_3(v_h__2_46_, v_x_43_, v_x_44_, crate::leanh::lean_box(0));
        return v___x_51_;
    }
}
pub unsafe fn l___private_Init_Data_Vector_Zip_0__Array_getElem_x3f__zipWith_match__1_splitter(
    mut v_00_u03b1_52_: *mut crate::leanh::LeanObject,
    mut v_00_u03b2_53_: *mut crate::leanh::LeanObject,
    mut v_motive_54_: *mut crate::leanh::LeanObject,
    mut v_x_55_: *mut crate::leanh::LeanObject,
    mut v_x_56_: *mut crate::leanh::LeanObject,
    mut v_h__1_57_: *mut crate::leanh::LeanObject,
    mut v_h__2_58_: *mut crate::leanh::LeanObject,
) -> *mut crate::leanh::LeanObject {
    if crate::leanh::lean_obj_tag(v_x_55_) == 1 {
        if crate::leanh::lean_obj_tag(v_x_56_) == 1 {
            let mut v_val_59_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            let mut v_val_60_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            let mut v___x_61_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            crate::leanh::lean_dec(v_h__2_58_);
            v_val_59_ = crate::leanh::lean_ctor_get(v_x_55_, 0);
            crate::leanh::lean_inc(v_val_59_);
            crate::leanh::lean_dec_ref_known(v_x_55_, 1);
            v_val_60_ = crate::leanh::lean_ctor_get(v_x_56_, 0);
            crate::leanh::lean_inc(v_val_60_);
            crate::leanh::lean_dec_ref_known(v_x_56_, 1);
            v___x_61_ = crate::leanh::lean_apply_2(v_h__1_57_, v_val_59_, v_val_60_);
            return v___x_61_;
        } else {
            let mut v___x_62_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            crate::leanh::lean_dec(v_h__1_57_);
            v___x_62_ =
                crate::leanh::lean_apply_3(v_h__2_58_, v_x_55_, v_x_56_, crate::leanh::lean_box(0));
            return v___x_62_;
        }
    } else {
        let mut v___x_63_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
        crate::leanh::lean_dec(v_h__1_57_);
        v___x_63_ =
            crate::leanh::lean_apply_3(v_h__2_58_, v_x_55_, v_x_56_, crate::leanh::lean_box(0));
        return v___x_63_;
    }
}
pub unsafe fn l___private_Init_Data_Vector_Zip_0__Vector_getElem_x3f__zipWith_match__1_splitter___redArg(
    mut v_x_64_: *mut crate::leanh::LeanObject,
    mut v_x_65_: *mut crate::leanh::LeanObject,
    mut v_h__1_66_: *mut crate::leanh::LeanObject,
    mut v_h__2_67_: *mut crate::leanh::LeanObject,
) -> *mut crate::leanh::LeanObject {
    if crate::leanh::lean_obj_tag(v_x_64_) == 1 {
        if crate::leanh::lean_obj_tag(v_x_65_) == 1 {
            let mut v_val_68_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            let mut v_val_69_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            let mut v___x_70_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            crate::leanh::lean_dec(v_h__2_67_);
            v_val_68_ = crate::leanh::lean_ctor_get(v_x_64_, 0);
            crate::leanh::lean_inc(v_val_68_);
            crate::leanh::lean_dec_ref_known(v_x_64_, 1);
            v_val_69_ = crate::leanh::lean_ctor_get(v_x_65_, 0);
            crate::leanh::lean_inc(v_val_69_);
            crate::leanh::lean_dec_ref_known(v_x_65_, 1);
            v___x_70_ = crate::leanh::lean_apply_2(v_h__1_66_, v_val_68_, v_val_69_);
            return v___x_70_;
        } else {
            let mut v___x_71_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            crate::leanh::lean_dec(v_h__1_66_);
            v___x_71_ =
                crate::leanh::lean_apply_3(v_h__2_67_, v_x_64_, v_x_65_, crate::leanh::lean_box(0));
            return v___x_71_;
        }
    } else {
        let mut v___x_72_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
        crate::leanh::lean_dec(v_h__1_66_);
        v___x_72_ =
            crate::leanh::lean_apply_3(v_h__2_67_, v_x_64_, v_x_65_, crate::leanh::lean_box(0));
        return v___x_72_;
    }
}
pub unsafe fn l___private_Init_Data_Vector_Zip_0__Vector_getElem_x3f__zipWith_match__1_splitter(
    mut v_00_u03b1_73_: *mut crate::leanh::LeanObject,
    mut v_00_u03b2_74_: *mut crate::leanh::LeanObject,
    mut v_motive_75_: *mut crate::leanh::LeanObject,
    mut v_x_76_: *mut crate::leanh::LeanObject,
    mut v_x_77_: *mut crate::leanh::LeanObject,
    mut v_h__1_78_: *mut crate::leanh::LeanObject,
    mut v_h__2_79_: *mut crate::leanh::LeanObject,
) -> *mut crate::leanh::LeanObject {
    if crate::leanh::lean_obj_tag(v_x_76_) == 1 {
        if crate::leanh::lean_obj_tag(v_x_77_) == 1 {
            let mut v_val_80_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            let mut v_val_81_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            let mut v___x_82_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            crate::leanh::lean_dec(v_h__2_79_);
            v_val_80_ = crate::leanh::lean_ctor_get(v_x_76_, 0);
            crate::leanh::lean_inc(v_val_80_);
            crate::leanh::lean_dec_ref_known(v_x_76_, 1);
            v_val_81_ = crate::leanh::lean_ctor_get(v_x_77_, 0);
            crate::leanh::lean_inc(v_val_81_);
            crate::leanh::lean_dec_ref_known(v_x_77_, 1);
            v___x_82_ = crate::leanh::lean_apply_2(v_h__1_78_, v_val_80_, v_val_81_);
            return v___x_82_;
        } else {
            let mut v___x_83_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
            crate::leanh::lean_dec(v_h__1_78_);
            v___x_83_ =
                crate::leanh::lean_apply_3(v_h__2_79_, v_x_76_, v_x_77_, crate::leanh::lean_box(0));
            return v___x_83_;
        }
    } else {
        let mut v___x_84_: *mut crate::leanh::LeanObject = core::ptr::null_mut();
        crate::leanh::lean_dec(v_h__1_78_);
        v___x_84_ =
            crate::leanh::lean_apply_3(v_h__2_79_, v_x_76_, v_x_77_, crate::leanh::lean_box(0));
        return v___x_84_;
    }
}
static mut _G_runtime_initialized: bool = false;
pub unsafe fn runtime_initialize_Init_Data_Vector_Zip(
    builtin: u8,
) -> *mut crate::leanh::LeanObject {
    let mut res: *mut crate::leanh::LeanObject = core::ptr::null_mut();
    if _G_runtime_initialized {
        return crate::leanh::lean_io_result_mk_ok(crate::leanh::lean_box(0));
    }
    _G_runtime_initialized = true;
    res = runtime_initialize_Init_Data_Array_Basic(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = runtime_initialize_Init_Data_Vector_Basic(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = runtime_initialize_Init_Data_Function(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = runtime_initialize_Init_Data_Vector_Basic(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = runtime_initialize_Init_Data_Array_Zip(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = runtime_initialize_Init_Data_Vector_Lemmas(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    return crate::leanh::lean_io_result_mk_ok(crate::leanh::lean_box(0));
}
static mut _G_meta_initialized: bool = false;
pub unsafe fn meta_initialize_Init_Data_Vector_Zip(builtin: u8) -> *mut crate::leanh::LeanObject {
    let mut res: *mut crate::leanh::LeanObject = core::ptr::null_mut();
    if _G_meta_initialized {
        return crate::leanh::lean_io_result_mk_ok(crate::leanh::lean_box(0));
    }
    _G_meta_initialized = true;
    return crate::leanh::lean_io_result_mk_ok(crate::leanh::lean_box(0));
}
static mut _G_initialized: bool = false;
pub unsafe fn initialize_Init_Data_Vector_Zip(builtin: u8) -> *mut crate::leanh::LeanObject {
    let mut res: *mut crate::leanh::LeanObject = core::ptr::null_mut();
    if _G_initialized {
        return crate::leanh::lean_io_result_mk_ok(crate::leanh::lean_box(0));
    }
    _G_initialized = true;
    res = initialize_Init_Data_Array_Basic(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = initialize_Init_Data_Vector_Basic(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = initialize_Init_Data_Function(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = initialize_Init_Data_Vector_Basic(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = initialize_Init_Data_Array_Zip(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = initialize_Init_Data_Vector_Lemmas(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = runtime_initialize_Init_Data_Vector_Zip(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    res = meta_initialize_Init_Data_Vector_Zip(builtin);
    if crate::leanh::lean_io_result_is_error(res) {
        return res;
    }
    crate::leanh::lean_dec_ref(res);
    return initialize_Init_Data_Vector_Zip(builtin);
}
