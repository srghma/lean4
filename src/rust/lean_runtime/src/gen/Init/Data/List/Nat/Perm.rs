...
// Imports: Init.GetElem Init.Data.List.Nat.TakeDrop Init.Data.List.Perm Init.Data.List.TakeDrop Init.Omega Init.TacticsExtra
...
pub unsafe fn runtime_initialize_Init_Data_List_Nat_Perm(builtin: u8) -> *mut LeanObject {
...
pub unsafe fn meta_initialize_Init_Data_List_Nat_Perm(builtin: u8) -> *mut LeanObject {
    let mut res: *mut LeanObject = core::ptr::null_mut();
...
pub unsafe fn initialize_Init_Data_List_Nat_Perm(builtin: u8) -> *mut LeanObject {
    let mut res: *mut LeanObject = core::ptr::null_mut();
    if _G_initialized {
        return lean_io_result_mk_ok(lean_box(0));
    }
    ...
    lean_dec_ref(res);
    res = meta_initialize_Init_Data_List_Nat_Perm(builtin);
    if lean_io_result_is_error(res) {
        return res;
    }
    lean_dec_ref(res);
    return initialize_Init_Data_List_Nat_Perm(builtin);
}
