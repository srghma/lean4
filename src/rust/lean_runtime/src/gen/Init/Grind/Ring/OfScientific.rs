...
// Imports: Init.Grind.Ring.Field Init.Data.OfScientific
...
pub unsafe fn runtime_initialize_Init_Grind_Ring_OfScientific(builtin: u8) -> *mut LeanObject {
...
pub unsafe fn meta_initialize_Init_Grind_Ring_OfScientific(builtin: u8) -> *mut LeanObject {
    let mut res: *mut LeanObject = core::ptr::null_mut();
...
pub unsafe fn initialize_Init_Grind_Ring_OfScientific(builtin: u8) -> *mut LeanObject {
    let mut res: *mut LeanObject = core::ptr::null_mut();
    if _G_initialized {
        return lean_io_result_mk_ok(lean_box(0));
    }
    ...
    lean_dec_ref(res);
    res = meta_initialize_Init_Grind_Ring_OfScientific(builtin);
    if lean_io_result_is_error(res) {
        return res;
    }
    lean_dec_ref(res);
    return initialize_Init_Grind_Ring_OfScientific(builtin);
}
