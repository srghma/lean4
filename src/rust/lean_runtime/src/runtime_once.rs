...
#[cfg(target_family = "unix")]
unsafe fn trace_dladdr(addr: *const core::ffi::c_void) {
    let mut info = core::mem::MaybeUninit::<DlInfo>::uninit();
    if dladdr(addr, info.as_mut_ptr()) == 0 {
        eprintln!("  dladdr: <unresolved>");
        return;
    }
    ...
    eprintln!(
        "  dladdr: file={} base={:p} addr={:p} symbol={}",
        fname,
        info.dli_fbase,
        addr,
        sname
...
pub unsafe extern "C" fn lean_obj_once_cold(
    ...
    let result = *loc;
    if trace {
        eprintln!("lean_obj_once_cold exit tok={:p} result={:p}", tok, result);
    }
