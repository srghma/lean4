...
// allocator (mimalloc or libc malloc) via the lean.h inline helpers.
...
mod runtime_alloc_impl {
    ...
    // -----------------------------------------------------------------------
    // External allocator primitives from lean.h / mimalloc
    // -----------------------------------------------------------------------

    extern "C" {
        fn lean_internal_panic_out_of_memory() -> !;
        // System alloc used when small allocator is off or for large objects.
        // These resolve to mimalloc or libc depending on build config.
    ...
    #[cfg(feature = "small_allocator")]
    #[export_name = "_ZN4lean17initialize_allocEv"]
    ...
    #[cfg(feature = "small_allocator")]
    #[export_name = "_ZN4lean15finalize_allocEv"]
    ...
    // (resolve to mimalloc / libc depending on build; defined in lean.h inlines
    //  but we need callable symbols from Rust)
    // -----------------------------------------------------------------------
    // These are declared `extern "C"` above; the definitions live in lean.h as
    // inline wrappers around mi_malloc / malloc that Clang/GCC will emit as
    // weak symbols.  If the linker doesn't find them, add thin C++ wrappers:
    //
    //   void * lean_sys_alloc(size_t sz) { return mi_malloc(sz); }
    //   void   lean_sys_free_sized(void * p, size_t sz) { mi_free_size(p, sz); }
}
