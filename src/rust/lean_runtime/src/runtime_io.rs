...
mod runtime_io_impl {
    ...
    extern "C" {
        ...
        fn lean_io_shim_get_stdin() -> *mut LeanObject;
        fn lean_io_shim_get_stdout() -> *mut LeanObject;
        fn lean_io_shim_get_stderr() -> *mut LeanObject;
        fn lean_io_shim_set_stdin(h: *mut LeanObject) -> *mut LeanObject;
        fn lean_io_shim_set_stdout(h: *mut LeanObject) -> *mut LeanObject;
        fn lean_io_shim_set_stderr(h: *mut LeanObject) -> *mut LeanObject;

        // io_wrap_handle / io_handle_external_class registration
        fn lean_io_shim_initialize(
            stdin_stream: *mut LeanObject,
            stdout_stream: *mut LeanObject,
            stderr_stream: *mut LeanObject,
        );
        fn lean_io_wrap_handle_c(fp: *mut libc_FILE) -> *mut LeanObject;
        ...
        // uv stat
        fn lean_io_shim_uv_stat(
            path: *const c_char,
            out: *mut UvStatBuf,
        ) -> c_int;
        fn lean_io_shim_uv_lstat(
            path: *const c_char,
            out: *mut UvStatBuf,
        ) -> c_int;
        fn lean_io_shim_uv_link(
            orig: *const c_char,
            link: *const c_char,
        ) -> c_int;
        fn lean_io_shim_uv_unlink(path: *const c_char) -> c_int;
        fn lean_io_shim_uv_strerror(errnum: c_int) -> *const c_char;
        fn lean_io_shim_uv_os_tmpdir(buf: *mut c_char, sz: *mut usize) -> c_int;
        fn lean_io_shim_uv_fs_mkstemp(
            pattern: *const c_char,
            out_fd: *mut c_int,
            out_path: *mut c_char,
            path_cap: usize,
        ) -> c_int;
        fn lean_io_shim_uv_fs_mkdtemp(
            pattern: *const c_char,
            out_path: *mut c_char,
            path_cap: usize,
        ) -> c_int;
    ...
    extern "C" {
        ...
        fn lean_array_mk_empty() -> *mut LeanObject;
        fn lean_int64_to_int(n: i64) -> *mut LeanObject;
        fn lean_io_eprintln(msg: *mut LeanObject) -> *mut LeanObject;
        fn lean_alloc_sarray_would_overflow(elem_size: usize, size: usize) -> bool;
    }
    ...
    /// LEAN_REF_TAG (254) — tag used for Ref objects
    const LEAN_REF_TAG_IO: u8 = 254;
    ...
    pub unsafe extern "C" fn lean_io_result_show_error(r: *mut LeanObject) {
        ...
        extern "C" { fn lean_io_shim_stderr() -> *mut libc_FILE; }
        fputs(b"uncaught exception: \0".as_ptr() as *const c_char, lean_io_shim_stderr());
        fputs(cstr, lean_io_shim_stderr());
        fputs(b"\n\0".as_ptr() as *const c_char, lean_io_shim_stderr());
        lean_dec(s);
    }
    ...
    pub unsafe extern "C" fn lean_io_prim_handle_mk(
        ...
        // Delegate to shim which knows the platform O_ constants
        extern "C" {
            fn lean_io_shim_open_handle(
                path: *const c_char,
                mode_u8: u8,
            ) -> *mut LeanObject; // returns IO Handle or IO Error
        }
        lean_io_shim_open_handle(fname, mode)
    }
    ...
    // -----------------------------------------------------------------------
    // Handle.lock / tryLock / unlock — delegate to shim (platform-specific)
    // -----------------------------------------------------------------------
    extern "C" {
        fn lean_io_shim_handle_lock(h: *mut LeanObject, exclusive: u8) -> *mut LeanObject;
        fn lean_io_shim_handle_try_lock(h: *mut LeanObject, exclusive: u8) -> *mut LeanObject;
        fn lean_io_shim_handle_unlock(h: *mut LeanObject) -> *mut LeanObject;
    }

    #[no_mangle]
    pub unsafe extern "C" fn lean_io_prim_handle_lock(h: *mut LeanObject, x: u8) -> *mut LeanObject {
        lean_io_shim_handle_lock(h, x)
    }
    #[no_mangle]
    pub unsafe extern "C" fn lean_io_prim_handle_try_lock(h: *mut LeanObject, x: u8) -> *mut LeanObject {
        lean_io_shim_handle_try_lock(h, x)
    }
    #[no_mangle]
    pub unsafe extern "C" fn lean_io_prim_handle_unlock(h: *mut LeanObject) -> *mut LeanObject {
        lean_io_shim_handle_unlock(h)
    }
    ...
    pub unsafe extern "C" fn lean_io_realpath(filename: *mut LeanObject) -> *mut LeanObject {
        extern "C" {
            fn lean_errno() -> c_int;
            fn lean_io_shim_realpath(path: *const c_char) -> *mut LeanObject; // platform impl
        }
        if string_has_embedded_nul(filename) {
        ...
        let fname = lean_string_cstr(filename);
        let result = lean_io_shim_realpath(fname);
        lean_dec(filename);
        result
    }
    ...
    pub unsafe extern "C" fn lean_io_metadata(filename: *mut LeanObject) -> *mut LeanObject {
        ...
        let ret = lean_io_shim_uv_stat(lean_string_cstr(filename), &mut st);
        if ret < 0 {
    ...
    pub unsafe extern "C" fn lean_io_symlink_metadata(filename: *mut LeanObject) -> *mut LeanObject {
        ...
        #[cfg(not(target_os = "windows"))]
        {
            ...
            let mut st = core::mem::zeroed::<UvStatBuf>();
            let ret = lean_io_shim_uv_lstat(lean_string_cstr(filename), &mut st);
            if ret < 0 {
                io_result_mk_error_obj(decode_uv_error(ret, filename))
            } else {
                metadata_core(&st)
    ...
    pub unsafe extern "C" fn lean_io_rename(
        ...
        extern "C" {
            fn lean_errno() -> c_int;
            fn lean_io_shim_rename(from: *const c_char, to: *const c_char) -> *mut LeanObject;
        }
        ...
        lean_io_shim_rename(lean_string_cstr(from), lean_string_cstr(to))
    }
    ...
    pub unsafe extern "C" fn lean_io_hard_link(
        ...
        let ret = lean_io_shim_uv_link(lean_string_cstr(orig), lean_string_cstr(link));
        if ret < 0 {
    ...
    pub unsafe extern "C" fn lean_io_remove_file(filename: *mut LeanObject) -> *mut LeanObject {
        ...
        let ret = lean_io_shim_uv_unlink(lean_string_cstr(filename));
        if ret < 0 {
            io_result_mk_error_obj(decode_uv_error(ret, filename))
        } else {
            io_result_mk_ok(lean_box(0))
    ...
    pub unsafe extern "C" fn lean_io_create_tempfile(_w: *mut LeanObject) -> *mut LeanObject {
        ...
        let ret = lean_io_shim_uv_os_tmpdir(base.as_mut_ptr() as *mut c_char, &mut base_len);
        if ret < 0 {
        ...
        let ret = lean_io_shim_uv_fs_mkstemp(
        ...
        let handle = lean_io_wrap_handle_c(fp);
    ...
    pub unsafe extern "C" fn lean_io_create_tempdir(_w: *mut LeanObject) -> *mut LeanObject {
        ...
        let mut out_path = [0u8; PATH_MAX];
        let ret = lean_io_shim_uv_fs_mkdtemp(
            base.as_ptr() as *const c_char,
            out_path.as_mut_ptr() as *mut c_char,
            PATH_MAX,
        );
    ...
    // -----------------------------------------------------------------------
    extern "C" {
        fn lean_io_shim_app_path() -> *mut LeanObject; // platform-specific
    }

    #[no_mangle]
    pub unsafe extern "C" fn lean_io_app_path() -> *mut LeanObject {
        lean_io_shim_app_path()
    }
    ...
    pub unsafe extern "C" fn lean_st_ref_get(rf: *mut LeanObject) -> *mut LeanObject {
        if ref_maybe_mt(rf) {
            extern "C" { fn lean_io_shim_ref_get_mt(rf: *mut LeanObject) -> *mut LeanObject; }
            lean_io_shim_ref_get_mt(rf)
        } else {
    ...
    pub unsafe extern "C" fn lean_st_ref_take(rf: *mut LeanObject) -> *mut LeanObject {
        if ref_maybe_mt(rf) {
            extern "C" { fn lean_io_shim_ref_take_mt(rf: *mut LeanObject) -> *mut LeanObject; }
            lean_io_shim_ref_take_mt(rf)
        } else {
            let val = (*(rf as *mut LeanRefObject)).m_value;
    ...
    pub unsafe extern "C" fn lean_st_ref_set(rf: *mut LeanObject, a: *mut LeanObject) -> *mut LeanObject {
        if ref_maybe_mt(rf) {
            lean_mark_mt(a);
            extern "C" { fn lean_io_shim_ref_set_mt(rf: *mut LeanObject, a: *mut LeanObject); }
            lean_io_shim_ref_set_mt(rf, a);
        } else {
    ...
    pub unsafe extern "C" fn lean_st_ref_swap(rf: *mut LeanObject, a: *mut LeanObject) -> *mut LeanObject {
        if ref_maybe_mt(rf) {
            lean_mark_mt(a);
            extern "C" { fn lean_io_shim_ref_swap_mt(rf: *mut LeanObject, a: *mut LeanObject) -> *mut LeanObject; }
            lean_io_shim_ref_swap_mt(rf, a)
        } else {
            let old = (*(rf as *mut LeanRefObject)).m_value;
    ...
    // -----------------------------------------------------------------------
    extern "C" {
        fn lean_io_shim_windows_get_next_transition(
            tz: *mut LeanObject,
            tm: u64,
            default_time: u8,
        ) -> *mut LeanObject;
        fn lean_io_shim_get_windows_local_timezone_id_at(tm: u64) -> *mut LeanObject;
    }
    ...
    pub unsafe extern "C" fn lean_windows_get_next_transition(
        ...
        lean_io_shim_windows_get_next_transition(timezone_str, tm_obj, default_time)
    }

    #[no_mangle]
    pub unsafe extern "C" fn lean_get_windows_local_timezone_id_at(tm: u64) -> *mut LeanObject {
        lean_io_shim_get_windows_local_timezone_id_at(tm)
    }
