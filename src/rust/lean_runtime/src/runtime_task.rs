...
impl TaskManager {
    ...
    fn spawn_worker(self: &Arc<Self>, guard: &mut MutexGuard<'_, TaskManagerInner>) {
        ...
        guard.idle_std_workers += 1; // will be incremented again below but net +0 from caller perspective; matches C++ pattern of incrementing before the wait
        // Actually mirror C++ exactly: worker starts and immediately increments idle.
        // We pre-count it here, then the thread will manage the counter itself.
        // Correction: set it up as the C++ does — spawn and the thread manages.
        guard.idle_std_workers -= 1; // undo; thread will do it
        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                // Throttle: don't exceed max_std_workers active workers
                // (unless shutting down).
                let active = guard.inner_worker_count(&guard);
                if !guard.shutting_down && active > guard.max_std_workers {
                    guard = tm.queue_cv.wait(guard).unwrap();
                    continue;
                }
...
impl TaskManagerInner {
    fn inner_worker_count(&self, _guard: &MutexGuard<'_, TaskManagerInner>) -> usize {
        // Active workers = total spawned - idle. We track idle directly.
        // We don't track total in Rust (C++ counts via m_std_workers.size()).
        // Use max_std_workers - idle as a proxy: spawn when idle == 0 and we
        // haven't saturated. The exact count is managed by the condvar logic.
        0 // placeholder; spawn decision is made by idle count check in enqueue_core
    }
}
...
unsafe fn lean_is_task(o: *mut LeanObject) -> bool {
    !lean_is_scalar(o) && (*o).tag == LEAN_TASK_TAG
}
