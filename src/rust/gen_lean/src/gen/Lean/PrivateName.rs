// Lean compiler output
// Module: Lean.PrivateName
// Imports: Init.Notation Init.Data.Option.Coe
use crate::ffi::{lean_name_eq, lean_nat_dec_eq};
use crate::r#gen::Init::Data::Option::Coe::{
    initialize_Init_Data_Option_Coe, runtime_initialize_Init_Data_Option_Coe,
};
use crate::r#gen::Init::Notation::{initialize_Init_Notation, runtime_initialize_Init_Notation};
use crate::r#gen::Init::Prelude::{
    l_Lean_Name_append, l_Lean_Name_num___override, l_Lean_Name_str___override,
};
pub static l_Lean_privateHeader___closed__0_value: leanh::LeanStringObject<9> =
    leanh::LeanStringObject {
        m_header: leanh::LeanObject {
            rc: 0,
            cs_size: (0) as u16,
            other: 0,
            tag: 249,
        },
        m_size: 9,
        m_capacity: 9,
        m_length: 8,
        m_data: [95, 112, 114, 105, 118, 97, 116, 101, 0],
    };
static mut l_Lean_privateHeader___closed__0: *mut leanh::LeanObject =
    core::ptr::addr_of!(l_Lean_privateHeader___closed__0_value) as *mut leanh::LeanObject;
pub static l_Lean_privateHeader___closed__1_value: leanh::LeanCtorObject<3> =
    leanh::LeanCtorObject {
        m_header: leanh::LeanObject {
            rc: 0,
            cs_size: (core::mem::size_of::<leanh::LeanObject>()
                + core::mem::size_of::<*mut leanh::LeanObject>() * 2
                + 8) as u16,
            other: 2,
            tag: 1,
        },
        m_objs: [
            (((0 as usize) << 1) | 1) as *mut leanh::LeanObject,
            core::ptr::addr_of!(l_Lean_privateHeader___closed__0_value)
                as *mut leanh::LeanObject,
            11079354408986465895 as *mut leanh::LeanObject,
        ],
    };
static mut l_Lean_privateHeader___closed__1: *mut leanh::LeanObject =
    core::ptr::addr_of!(l_Lean_privateHeader___closed__1_value) as *mut leanh::LeanObject;
pub static mut l_Lean_privateHeader: *mut leanh::LeanObject =
    core::ptr::addr_of!(l_Lean_privateHeader___closed__1_value) as *mut leanh::LeanObject;
pub unsafe fn l_Lean_mkPrivateNameCore(
    mut v_mainModule_79_: *mut leanh::LeanObject,
    mut v_n_80_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    let mut v___x_81_: *mut leanh::LeanObject = core::ptr::null_mut();
    let mut v___x_82_: *mut leanh::LeanObject = core::ptr::null_mut();
    let mut v___x_83_: *mut leanh::LeanObject = core::ptr::null_mut();
    let mut v___x_84_: *mut leanh::LeanObject = core::ptr::null_mut();
    let mut v___x_85_: *mut leanh::LeanObject = core::ptr::null_mut();
    v___x_81_ = l_Lean_privateHeader;
    v___x_82_ = l_Lean_Name_append(v___x_81_, v_mainModule_79_);
    v___x_83_ = leanh::lean_unsigned_to_nat(0);
    v___x_84_ = l_Lean_Name_num___override(v___x_82_, v___x_83_);
    v___x_85_ = l_Lean_Name_append(v___x_84_, v_n_80_);
    return v___x_85_;
}
pub unsafe fn l_Lean_isPrivateName(mut v_x_86_: *mut leanh::LeanObject) -> u8 {
    let mut v___x_87_: u8 = 0;
    let mut v_pre_88_: *mut leanh::LeanObject = core::ptr::null_mut();
    let mut v___x_89_: *mut leanh::LeanObject = core::ptr::null_mut();
    let mut v___x_90_: u8 = 0;
    let mut v_pre_92_: *mut leanh::LeanObject = core::ptr::null_mut();
    let mut state = 0;
    loop {
        match state {
            0 => match leanh::lean_obj_tag(v_x_86_) {
                0 => {
                    v___x_87_ = 0;
                    return v___x_87_;
                }
                1 => {
                    v_pre_88_ = leanh::lean_ctor_get(v_x_86_, 0);
                    v___x_89_ = l_Lean_privateHeader;
                    v___x_90_ = lean_name_eq(v_x_86_, v___x_89_);
                    if v___x_90_ == 0 {
                        v_x_86_ = v_pre_88_;
                        state = 0;
                        continue;
                    } else {
                        return v___x_90_;
                    }
                }
                _ => {
                    v_pre_92_ = leanh::lean_ctor_get(v_x_86_, 0);
                    v_x_86_ = v_pre_92_;
                    state = 0;
                    continue;
                }
            },
            _ => {}
        }
    }
}
pub unsafe fn l_Lean_isPrivateName___boxed(
    mut v_x_94_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    let mut v_res_95_: u8 = 0;
    let mut v_r_96_: *mut leanh::LeanObject = core::ptr::null_mut();
    v_res_95_ = l_Lean_isPrivateName(v_x_94_);
    leanh::lean_dec(v_x_94_);
    v_r_96_ = leanh::lean_box((v_res_95_) as usize);
    return v_r_96_;
}
pub unsafe fn lean_is_private_name(mut v_n_97_: *mut leanh::LeanObject) -> u8 {
    let mut v___x_98_: u8 = 0;
    v___x_98_ = l_Lean_isPrivateName(v_n_97_);
    leanh::lean_dec(v_n_97_);
    return v___x_98_;
}
pub unsafe fn l_Lean_isPrivateNameExport___boxed(
    mut v_n_99_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    let mut v_res_100_: u8 = 0;
    let mut v_r_101_: *mut leanh::LeanObject = core::ptr::null_mut();
    v_res_100_ = lean_is_private_name(v_n_99_);
    v_r_101_ = leanh::lean_box((v_res_100_) as usize);
    return v_r_101_;
}
pub unsafe fn l___private_Lean_PrivateName_0__Lean_isPrivatePrefix_go(
    mut v_n_102_: *mut leanh::LeanObject,
) -> u8 {
    let mut v___x_103_: *mut leanh::LeanObject = core::ptr::null_mut();
    let mut v___x_104_: u8 = 0;
    let mut v_pre_105_: *mut leanh::LeanObject = core::ptr::null_mut();
    let mut state = 0;
    loop {
        match state {
            0 => {
                v___x_103_ = l_Lean_privateHeader;
                v___x_104_ = lean_name_eq(v_n_102_, v___x_103_);
                if v___x_104_ == 0 {
                    if leanh::lean_obj_tag(v_n_102_) == 1 {
                        v_pre_105_ = leanh::lean_ctor_get(v_n_102_, 0);
                        v_n_102_ = v_pre_105_;
                        state = 0;
                        continue;
                    } else {
                        return v___x_104_;
                    }
                } else {
                    return v___x_104_;
                }
            }
            _ => {}
        }
    }
}
pub unsafe fn l___private_Lean_PrivateName_0__Lean_isPrivatePrefix_go___boxed(
    mut v_n_107_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    let mut v_res_108_: u8 = 0;
    let mut v_r_109_: *mut leanh::LeanObject = core::ptr::null_mut();
    v_res_108_ = l___private_Lean_PrivateName_0__Lean_isPrivatePrefix_go(v_n_107_);
    leanh::lean_dec(v_n_107_);
    v_r_109_ = leanh::lean_box((v_res_108_) as usize);
    return v_r_109_;
}
pub unsafe fn l_Lean_isPrivatePrefix(mut v_n_110_: *mut leanh::LeanObject) -> u8 {
    if leanh::lean_obj_tag(v_n_110_) == 2 {
        let mut v_pre_111_: *mut leanh::LeanObject = core::ptr::null_mut();
        let mut v_i_112_: *mut leanh::LeanObject = core::ptr::null_mut();
        let mut v___x_113_: *mut leanh::LeanObject = core::ptr::null_mut();
        let mut v___x_114_: u8 = 0;
        v_pre_111_ = leanh::lean_ctor_get(v_n_110_, 0);
        v_i_112_ = leanh::lean_ctor_get(v_n_110_, 1);
        v___x_113_ = leanh::lean_unsigned_to_nat(0);
        v___x_114_ = lean_nat_dec_eq(v_i_112_, v___x_113_);
        if v___x_114_ == 0 {
            return v___x_114_;
        } else {
            let mut v___x_115_: u8 = 0;
            v___x_115_ = l___private_Lean_PrivateName_0__Lean_isPrivatePrefix_go(v_pre_111_);
            return v___x_115_;
        }
    } else {
        let mut v___x_116_: u8 = 0;
        v___x_116_ = 0;
        return v___x_116_;
    }
}
pub unsafe fn l_Lean_isPrivatePrefix___boxed(
    mut v_n_117_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    let mut v_res_118_: u8 = 0;
    let mut v_r_119_: *mut leanh::LeanObject = core::ptr::null_mut();
    v_res_118_ = l_Lean_isPrivatePrefix(v_n_117_);
    leanh::lean_dec(v_n_117_);
    v_r_119_ = leanh::lean_box((v_res_118_) as usize);
    return v_r_119_;
}
pub unsafe fn l___private_Lean_PrivateName_0__Lean_privateToUserNameAux(
    mut v_n_120_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    match leanh::lean_obj_tag(v_n_120_) {
        0 => {
            return v_n_120_;
        }
        1 => {
            let mut v_pre_121_: *mut leanh::LeanObject = core::ptr::null_mut();
            let mut v_str_122_: *mut leanh::LeanObject = core::ptr::null_mut();
            let mut v___x_123_: *mut leanh::LeanObject = core::ptr::null_mut();
            let mut v___x_124_: *mut leanh::LeanObject = core::ptr::null_mut();
            v_pre_121_ = leanh::lean_ctor_get(v_n_120_, 0);
            leanh::lean_inc(v_pre_121_);
            v_str_122_ = leanh::lean_ctor_get(v_n_120_, 1);
            leanh::lean_inc_ref(v_str_122_);
            leanh::lean_dec_ref_known(v_n_120_, 2);
            v___x_123_ = l___private_Lean_PrivateName_0__Lean_privateToUserNameAux(v_pre_121_);
            v___x_124_ = l_Lean_Name_str___override(v___x_123_, v_str_122_);
            return v___x_124_;
        }
        _ => {
            let mut v_pre_125_: *mut leanh::LeanObject = core::ptr::null_mut();
            let mut v_i_126_: *mut leanh::LeanObject = core::ptr::null_mut();
            let mut v___x_127_: u8 = 0;
            v_pre_125_ = leanh::lean_ctor_get(v_n_120_, 0);
            leanh::lean_inc(v_pre_125_);
            v_i_126_ = leanh::lean_ctor_get(v_n_120_, 1);
            leanh::lean_inc(v_i_126_);
            v___x_127_ = l_Lean_isPrivatePrefix(v_n_120_);
            leanh::lean_dec_ref_known(v_n_120_, 2);
            if v___x_127_ == 0 {
                let mut v___x_128_: *mut leanh::LeanObject = core::ptr::null_mut();
                let mut v___x_129_: *mut leanh::LeanObject = core::ptr::null_mut();
                v___x_128_ = l___private_Lean_PrivateName_0__Lean_privateToUserNameAux(v_pre_125_);
                v___x_129_ = l_Lean_Name_num___override(v___x_128_, v_i_126_);
                return v___x_129_;
            } else {
                let mut v___x_130_: *mut leanh::LeanObject = core::ptr::null_mut();
                leanh::lean_dec(v_i_126_);
                leanh::lean_dec(v_pre_125_);
                v___x_130_ = leanh::lean_box(0);
                return v___x_130_;
            }
        }
    }
}
pub unsafe fn lean_private_to_user_name(
    mut v_n_131_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    let mut v___x_132_: u8 = 0;
    v___x_132_ = l_Lean_isPrivateName(v_n_131_);
    if v___x_132_ == 0 {
        let mut v___x_133_: *mut leanh::LeanObject = core::ptr::null_mut();
        leanh::lean_dec(v_n_131_);
        v___x_133_ = leanh::lean_box(0);
        return v___x_133_;
    } else {
        let mut v___x_134_: *mut leanh::LeanObject = core::ptr::null_mut();
        let mut v___x_135_: *mut leanh::LeanObject = core::ptr::null_mut();
        v___x_134_ = l___private_Lean_PrivateName_0__Lean_privateToUserNameAux(v_n_131_);
        v___x_135_ = leanh::lean_alloc_ctor(1, 1, (0) as u32);
        leanh::lean_ctor_set(v___x_135_, 0, v___x_134_);
        return v___x_135_;
    }
}
pub unsafe fn l_Lean_privateToUserName(
    mut v_n_136_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    let mut v___x_137_: u8 = 0;
    v___x_137_ = l_Lean_isPrivateName(v_n_136_);
    if v___x_137_ == 0 {
        return v_n_136_;
    } else {
        let mut v___x_138_: *mut leanh::LeanObject = core::ptr::null_mut();
        v___x_138_ = l___private_Lean_PrivateName_0__Lean_privateToUserNameAux(v_n_136_);
        return v___x_138_;
    }
}
pub unsafe fn l___private_Lean_PrivateName_0__Lean_privatePrefixAux(
    mut v_x_139_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    let mut v_pre_140_: *mut leanh::LeanObject = core::ptr::null_mut();
    let mut state = 0;
    loop {
        match state {
            0 => {
                if leanh::lean_obj_tag(v_x_139_) == 1 {
                    v_pre_140_ = leanh::lean_ctor_get(v_x_139_, 0);
                    v_x_139_ = v_pre_140_;
                    state = 0;
                    continue;
                } else {
                    leanh::lean_inc(v_x_139_);
                    return v_x_139_;
                }
            }
            _ => {}
        }
    }
}
pub unsafe fn l___private_Lean_PrivateName_0__Lean_privatePrefixAux___boxed(
    mut v_x_142_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    let mut v_res_143_: *mut leanh::LeanObject = core::ptr::null_mut();
    v_res_143_ = l___private_Lean_PrivateName_0__Lean_privatePrefixAux(v_x_142_);
    leanh::lean_dec(v_x_142_);
    return v_res_143_;
}
pub unsafe fn lean_private_prefix(
    mut v_n_144_: *mut leanh::LeanObject,
) -> *mut leanh::LeanObject {
    let mut v___x_145_: u8 = 0;
    v___x_145_ = l_Lean_isPrivateName(v_n_144_);
    if v___x_145_ == 0 {
        let mut v___x_146_: *mut leanh::LeanObject = core::ptr::null_mut();
        leanh::lean_dec(v_n_144_);
        v___x_146_ = leanh::lean_box(0);
        return v___x_146_;
    } else {
        let mut v___x_147_: *mut leanh::LeanObject = core::ptr::null_mut();
        let mut v___x_148_: *mut leanh::LeanObject = core::ptr::null_mut();
        v___x_147_ = l___private_Lean_PrivateName_0__Lean_privatePrefixAux(v_n_144_);
        leanh::lean_dec(v_n_144_);
        v___x_148_ = leanh::lean_alloc_ctor(1, 1, (0) as u32);
        leanh::lean_ctor_set(v___x_148_, 0, v___x_147_);
        return v___x_148_;
    }
}
static mut _G_runtime_initialized: bool = false;
pub unsafe fn runtime_initialize_Lean_PrivateName(builtin: u8) -> *mut leanh::LeanObject {
    let mut res: *mut leanh::LeanObject = core::ptr::null_mut();
    if _G_runtime_initialized {
        return leanh::lean_io_result_mk_ok(leanh::lean_box(0));
    }
    _G_runtime_initialized = true;
    res = runtime_initialize_Init_Notation(builtin);
    if leanh::lean_io_result_is_error(res) {
        return res;
    }
    leanh::lean_dec_ref(res);
    res = runtime_initialize_Init_Data_Option_Coe(builtin);
    if leanh::lean_io_result_is_error(res) {
        return res;
    }
    leanh::lean_dec_ref(res);
    return leanh::lean_io_result_mk_ok(leanh::lean_box(0));
}
static mut _G_meta_initialized: bool = false;
pub unsafe fn meta_initialize_Lean_PrivateName(builtin: u8) -> *mut leanh::LeanObject {
    let mut res: *mut leanh::LeanObject = core::ptr::null_mut();
    if _G_meta_initialized {
        return leanh::lean_io_result_mk_ok(leanh::lean_box(0));
    }
    _G_meta_initialized = true;
    return leanh::lean_io_result_mk_ok(leanh::lean_box(0));
}
static mut _G_initialized: bool = false;
pub unsafe fn initialize_Lean_PrivateName(builtin: u8) -> *mut leanh::LeanObject {
    let mut res: *mut leanh::LeanObject = core::ptr::null_mut();
    if _G_initialized {
        return leanh::lean_io_result_mk_ok(leanh::lean_box(0));
    }
    _G_initialized = true;
    res = initialize_Init_Notation(builtin);
    if leanh::lean_io_result_is_error(res) {
        return res;
    }
    leanh::lean_dec_ref(res);
    res = initialize_Init_Data_Option_Coe(builtin);
    if leanh::lean_io_result_is_error(res) {
        return res;
    }
    leanh::lean_dec_ref(res);
    res = runtime_initialize_Lean_PrivateName(builtin);
    if leanh::lean_io_result_is_error(res) {
        return res;
    }
    leanh::lean_dec_ref(res);
    res = meta_initialize_Lean_PrivateName(builtin);
    if leanh::lean_io_result_is_error(res) {
        return res;
    }
    leanh::lean_dec_ref(res);
    return initialize_Lean_PrivateName(builtin);
}