...
pub unsafe fn l_Lean_Meta_Simp_Arith_parentIsTarget(
    mut v_parent_x3f_12_: *mut leanh::LeanObject,
) -> u8 {
    let mut v___x_13_: u8 = 0;
    let mut v_val_14_: *mut leanh::LeanObject = core::ptr::null_mut();
    ...
    loop {
        match state {
            0 => {
                if leanh::lean_obj_tag(v_parent_x3f_12_) == 0 {
                    v___x_13_ = 0;
                    return v___x_13_;
                } else {
                    v_val_14_ = leanh::lean_ctor_get(v_parent_x3f_12_, 0);
                    leanh::lean_inc_n(v_val_14_, 2);
                    leanh::lean_dec_ref_known(v_parent_x3f_12_, 1);
                    v___x_18_ = l_Lean_Meta_Simp_Arith_isLinearTerm(v_val_14_);
                    if v___x_18_ == 0 {
                        leanh::lean_inc(v_val_14_);
...
pub unsafe fn l_Lean_Meta_Simp_Arith_parentIsTarget___boxed(
    mut v_parent_x3f_20_: *mut leanh::LeanObject,
...
pub unsafe fn initialize_Lean_Meta_Tactic_Simp_Arith(builtin: u8) -> *mut leanh::LeanObject {
    let mut res: *mut leanh::LeanObject = core::ptr::null_mut();
    if _G_initialized {
        return leanh::lean_io_result_mk_ok(leanh::lean_box(0));
    }
